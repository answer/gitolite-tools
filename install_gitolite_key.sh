...
ssh-keygen -f ~/.ssh/$gitolite_keydir/$user && scp ~/.ssh/$gitolite_keydir/$user.pub $gitolite_host:. && ssh $gitolite_host 'sh -c '"'"'mkdir -p ~/gitolite-admin/keydir/$0/$1 && mv ~/$0.pub ~/gitolite-admin/keydir/$0/$1/$0.pub && cd ~/gitolite-admin && git pull && git add keydir/$0/$1/$0.pub && git commit -m "add $0.$1 key" && git push'"' $user `hostname`"
