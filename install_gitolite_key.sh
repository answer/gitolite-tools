...
ssh-keygen -f ~/.ssh/$gitolite_keydir/$user && scp ~/.ssh/$gitolite_keydir/$user.pub $gitolite_host:. && ssh $gitolite_host 'sh -c '"'"'mkdir -p ~/gitolite-admin/keydir/$0/$2/$1 && mv ~/$1.pub ~/gitolite-admin/keydir/$0/$2/$1.pub && cd ~/gitolite-admin && git pull && git add . && git commit -m "add $0.$1 key" && git push'"' $USER $user `hostname`"
