...
ssh-keygen -f ~/.ssh/$gitolite_keydir/$user && sh -c 'mkdir -p ~/gitolite-admin/keydir/$0/$1/$2 && cp ~/.ssh/$3/$1.pub ~/gitolite-admin/keydir/$0/$2/$1.pub && cd ~/gitolite-admin && git pull && git add . && git commit -m "add $0.$1 key" && git push' $USER $user `hostname` $gitolite_keydir
